...
def diamond_to_pp_count(diamond,genomes):
    pp = pd.DataFrame({'query':diamond.qseqid,
                       'genome':diamond.sseqid.map(lambda x: x.split('|')[0]),
                       'value':diamond.sseqid})
#     pp = pp.drop_duplicates()
#     print(pp.head())
    pp = pp.groupby(['query','genome'])['value'].agg('count').reset_index()
#     print(pp.head())
...
if __name__ == '__main__':
    ...
    fileout_pp = args.output_dir + '/' + prefix+ '_pp_count.pickle'
    ...
    if not os.path.exists(fileout_pp):
        ...
        pp = diamond_to_pp_count(diamond,genomes)
